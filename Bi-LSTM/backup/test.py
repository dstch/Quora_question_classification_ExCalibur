...
pred = bi_lstm().model(128, input_data, weights, biases)
...
vector_batch, label_batch = tf.train.batch([vector, label], batch_size=32, num_threads=4, capacity=256)
...
tfrecord_filename = '../train_data/train_word_id.tf_record'
filename_queue = tf.train.string_input_producer([tfrecord_filename], num_epochs=2)
reader = tf.TFRecordReader()
_, serialized_example = reader.read(filename_queue)
features = tf.parse_single_example(serialized_example, features={
    "features": tf.FixedLenFeature([15], tf.int64),
...
train_vector_batch, label_batch = tf.train.batch([vector, label], batch_size=32, num_threads=4, capacity=256,
                                                 allow_smaller_final_batch=True)

indices = tf.expand_dims(tf.range(0, 32, 1), 1)
concated = tf.concat([indices, label_batch], 1)
train_onehot_labels = tf.sparse_to_dense(concated, tf.stack([32, 2]), 1.0, 0.0)
...
with tf.Session() as sess:
    ...
    train_vector_batch = tf.nn.embedding_lookup(W, train_vector_batch)
    ...
    try:
        while not coord.should_stop():
            ...
            train_example, train_l = sess.run([train_vector_batch, train_onehot_labels])
            # tf.logging.info("start %s step optimizer" % step)
            sess.run(optimizer, feed_dict={
                input_data: train_example,
                y: train_l
            })
            if step % 1000 == 0:
                example, l = sess.run([vector_batch, onehot_labels])
                # Calculate batch accuracy
                acc = sess.run(accuracy, feed_dict={input_data: train_example, y: train_l})
                # Calculate batch loss
                loss = sess.run(cost, feed_dict={input_data: train_example, y: train_l})
